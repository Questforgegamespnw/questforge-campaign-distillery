...
import IntroductionsView from "./IntroductionsView";
  ...
  return (
    <section className="matchmaking-content">
      ...
      <div className="metric-grid">
        ...
        <Metric label="Introductions" value={overview?.counts?.introductions} />
...
function PairDetail({ pair, onCreateIntroduction }) {
  ...
  return <div className="detail-stack">
    ...
    {pair.eligibility?.eligible && (
      <div className="detail-actions">
        <button className="primary" onClick={() => onCreateIntroduction(pair)}>
          Create Introduction Draft
        </button>
      </div>
    )}
...
function Pairs({ pairs, selectedId, onSelect, detail, onCreateIntroduction }) {
  return <div className="split-view">
    ...
    </section><aside className="detail-panel"><PairDetail pair={detail} onCreateIntroduction={onCreateIntroduction} /></aside>
  </div>;
}
...
function Groups({ profiles, result, onBuild, onCreateIntroduction }) {
          ...
          {!result ? (
            ...
            <>
              ...
              {result.eligibility?.eligible && (
                <button
                  className="primary"
                  onClick={() => onCreateIntroduction(result)}
                >
                  Create Introduction Draft
                </button>
              )}
...
export default function MatchmakingWorkspace({ onStatusChange }) {
  ...
  async function refresh() {
    setBusy(true);
    try {
      const [nextOverview, nextProfiles, nextPairs, nextIntroductions] = await Promise.all([
        api.getMatchmakingOverview(),
        api.listMatchmakingProfiles(),
        api.listPairEvaluations(),
        api.listIntroductionRecords()
      ]);
      setOverview(nextOverview);
      setProfiles(nextProfiles.profiles || []);
      setPairs(nextPairs.evaluations || []);
      setIntroductions(nextIntroductions.records || []);
  ...
  useEffect(() => {
    ...
    api.getIntroductionRecord(selectedIntroduction)
      .then(setIntroductionDetail)
      .catch((e) => setMessage(e.message));
  }, [selectedIntroduction]);

  async function refreshIntroductions(preferredId = "") {
    const result = await api.listIntroductionRecords();
    setIntroductions(result.records || []);
    const nextId = preferredId || selectedIntroduction;
    if (nextId) {
      setSelectedIntroduction(nextId);
      setIntroductionDetail(await api.getIntroductionRecord(nextId));
    }
  }

  async function createIntroduction(sourceMatch) {
    setBusy(true);
    try {
      const saved = await api.createIntroductionDraft(sourceMatch);
      setSelectedIntroduction(saved.record.introductionId);
      setIntroductionDetail(saved);
      setTab("introductions");
      await refresh();
      setMessage("Introduction draft created.");
    } catch (error) {
  ...
  return <div className="matchmaking-workspace"><aside className="matchmaking-nav">{tabs.map(([id, label]) => <button key={id} className={tab === id ? "active" : ""} onClick={() => setTab(id)}>{label}</button>)}</aside><div className="matchmaking-main">{message && <div className="error-banner">{message}</div>}{tab === "overview" && <Overview
        ...
        busy={busy}
      />}{tab === "profiles" && <Profiles profiles={profiles} selectedId={selectedProfile} onSelect={setSelectedProfile} detail={profileDetail} onCompare={compare} />}{tab === "pairs" && (
        <Pairs
          pairs={pairs}
          selectedId={selectedPair}
          onSelect={setSelectedPair}
          detail={pairDetail}
          onCreateIntroduction={createIntroduction}
        />
      )}
      {tab === "groups" && (
        <Groups
      ...
      {tab === "introductions" && (
        <IntroductionsView
          ...
          selectedId={selectedIntroduction}
          onSelect={setSelectedIntroduction}
          detail={introductionDetail}
          onRefresh={refreshIntroductions}
        />
      )}</div></div>;
}
