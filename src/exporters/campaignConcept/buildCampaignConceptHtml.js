...
function renderSignalList(values = []) {
  if (!Array.isArray(values) || values.length === 0) {
    return "—";
  }

  return values.map(formatSignalLabel).filter(Boolean).join(", ") || "—";
}

function hasGenreContext(genreContext = {}) {
  return [
    genreContext.legacyGenre,
    genreContext.eras,
    genreContext.aesthetics,
    genreContext.worldConditions
  ].some((entries) => Array.isArray(entries) && entries.length > 0);
}

function renderGenreContextPanel(genreContext = {}) {
  if (!hasGenreContext(genreContext)) {
    return "";
  }

  return `
    <section class="implementation-context-panel">
      <div class="section-label">Implementation Context</div>
      <p class="context-note">These signals guide concrete Phase 2 implementation details. They do not replace the approved campaign identity.</p>
      <div class="context-signal-grid">
        <div><strong>Genre flavor</strong><span>${escapeHtml(renderSignalList(genreContext.legacyGenre))}</span></div>
        <div><strong>Era signals</strong><span>${escapeHtml(renderSignalList(genreContext.eras))}</span></div>
        <div><strong>Aesthetic signals</strong><span>${escapeHtml(renderSignalList(genreContext.aesthetics))}</span></div>
        <div><strong>World condition signals</strong><span>${escapeHtml(renderSignalList(genreContext.worldConditions))}</span></div>
      </div>
    </section>`;
}
...
function buildCampaignConceptHtml(
  ...
  const genreContextPanel = renderGenreContextPanel(
    data.genreContext || {}
  );
